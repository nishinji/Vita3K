...
bool convert_color_format_to_texture_format(SceGxmColorFormat format, SceGxmTextureFormat &dest_format) {
    struct ColorToTextureFormat {
        uint32_t color;
        uint32_t texture;
    };

    static constexpr auto COLOR_FORMAT_TO_TEXTURE_MAPPING = [] {
        auto table = std::to_array<ColorToTextureFormat>({ { SCE_GXM_COLOR_FORMAT_U8U8U8U8_ABGR, SCE_GXM_TEXTURE_FORMAT_U8U8U8U8_ABGR },
        ...
        std::ranges::sort(table, {}, &ColorToTextureFormat::color);
        return table;
    }();
    static_assert(std::ranges::adjacent_find(COLOR_FORMAT_TO_TEXTURE_MAPPING, {}, &ColorToTextureFormat::color) == COLOR_FORMAT_TO_TEXTURE_MAPPING.end(), "duplicate color format");

    const auto ite = std::ranges::lower_bound(COLOR_FORMAT_TO_TEXTURE_MAPPING, static_cast<uint32_t>(format), {}, &ColorToTextureFormat::color);
    if (ite == COLOR_FORMAT_TO_TEXTURE_MAPPING.end() || ite->color != static_cast<uint32_t>(format))
        return false;

    dest_format = static_cast<SceGxmTextureFormat>(ite->texture);
    return true;
}
} // namespace gxm
