...
SettingsDialogTooltips::SettingsDialogTooltips(QObject *parent)
    ...
    , async_pipeline(tr("Compile shaders in the background instead of blocking rendering.\nThis decreases compilation stutter at the cost of temporary graphical glitches.\nOn Vulkan, whole pipelines are compiled on worker threads.\nOn OpenGL, shaders are translated on worker threads, and linking is also\noffloaded to the driver when it supports GL_ARB_parallel_shader_compile."))
