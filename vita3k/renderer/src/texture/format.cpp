...
bool convert_base_texture_format_to_base_color_format(SceGxmTextureBaseFormat format, SceGxmColorBaseFormat &color_format) {
    struct TextureToColorFormat {
        uint32_t texture;
        uint32_t color;
    };

    static constexpr auto TEXTURE_TO_COLOR_FORMAT_MAPPING = [] {
        auto table = std::to_array<TextureToColorFormat>({ { SCE_GXM_TEXTURE_BASE_FORMAT_U8U8U8U8, SCE_GXM_COLOR_BASE_FORMAT_U8U8U8U8 },
        ...
        std::ranges::sort(table, {}, &TextureToColorFormat::texture);
        return table;
    }();
    static_assert(std::ranges::adjacent_find(TEXTURE_TO_COLOR_FORMAT_MAPPING, {}, &TextureToColorFormat::texture) == TEXTURE_TO_COLOR_FORMAT_MAPPING.end(), "duplicate texture format");

    const auto ite = std::ranges::lower_bound(TEXTURE_TO_COLOR_FORMAT_MAPPING, static_cast<uint32_t>(format), {}, &TextureToColorFormat::texture);
    if (ite == TEXTURE_TO_COLOR_FORMAT_MAPPING.end() || ite->texture != static_cast<uint32_t>(format))
        return false;

    color_format = static_cast<SceGxmColorBaseFormat>(ite->color);
    return true;
}
